...
echo "SubscriptionName,PolicyName,RetentionDays,RetentionMonths,RunFreq,IdSegment,RecoveryServicesVault,SnapshotRetentionDays,SnapshotType" > policy.csv
            ...
            if [ -n "$vaultExists" ]; then
                ...
                for policy in $(echo "${policies}" | jq -r '.[] | @base64'); do
                    ...
                    if [[ $policyName == VirtualMachine-Tier* ]]; then
                        ...
                        policyDetails=$(az backup policy show --resource-group "$resourceGroup" --vault-name "$vault" --name "$policyName" --query "{PolicyName: name, RetentionDays: properties.retentionPolicy.dailySchedule.retentionDuration.count, RetentionMonths: properties.retentionPolicy.monthlySchedule.retentionDuration.count, RunFreq: properties.schedulePolicy.scheduleRunFrequency, Id: id, SnapshotRetentionDays: properties.instantRpRetentionRangeInDays, SnapshotType: properties.snapshotType}" --output json 2>/dev/null)
                        if [ -n "$policyDetails" ]; then
                            ...
                            echo "$subscriptionName,$policyName,$retentionDays,$retentionMonths,$runFreq,$idSegment,$vault,$snapshotRetentionDays,$snapshotType" >> policy.csv
